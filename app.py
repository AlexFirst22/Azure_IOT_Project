...
# ─────────────────────────────────────────────────────────────────────────────
# POMOCNÉ FUNKCIE — Ukladanie prevodov do JSON súboru
# ─────────────────────────────────────────────────────────────────────────────
# Prevody sa ukladajú do súboru prevody.json (nie do databázy).
# Každý záznam je slovník: hodnota, typ, vysledok, popis, cas
...
def nacitaj_prevody():
    """
    Načíta zoznam všetkých prevodov zo súboru prevody.json.
    Ak súbor neexistuje (prvé spustenie), vráti prázdny zoznam.
    """
...
def uloz_prevod(zaznam):
    """
    Pridá nový záznam do prevody.json.
    Načíta existujúci zoznam, pridá nový prvok a uloží späť.

    Parametre:
        zaznam (dict): Slovník s kľúčmi hodnota, typ, vysledok, popis, cas
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# ROUTE: /api/prevod — Prevodník jednotiek pre IoT senzory
# ─────────────────────────────────────────────────────────────────────────────
# URL: /api/prevod?hodnota=100&typ=c_to_f
# Metóda: GET
#
# Podporované prevody:
#   c_to_f     = stupne Celzia na Fahrenheit (teplota)
#   hpa_to_mmhg = hektopascaly na milimetre ortuti (tlak)
#   ms_to_kmh  = metre za sekundu na kilometre za hodinu (rýchlosť vetra)

@app.route("/api/prevod")
def prevod():
    """
    Prijme číselnú hodnotu a typ prevodu, vykoná výpočet,
    uloží výsledok do prevody.json a vráti JSON odpoveď.

    Príklad URL:
        /api/prevod?hodnota=25&typ=c_to_f

    Výstup (JSON):
        {"hodnota": 25, "typ": "c_to_f", "vysledok": 77.0, "popis": "25 °C = 77.00 °F", "cas": "..."}
    """
    # --- Krok 1: Čítanie parametrov z URL ---
    ...
    # --- Krok 4: Zostavenie záznamu a uloženie do súboru ---
    zaznam = {
        "hodnota": hodnota,
        "typ": typ,
    ...
    # --- Krok 5: Vrátenie výsledku ---
    return jsonify(zaznam)


# ─────────────────────────────────────────────────────────────────────────────
# ROUTE: /api/historia-prevodov — História všetkých prevodov
# ─────────────────────────────────────────────────────────────────────────────
# URL: /api/historia-prevodov
# Metóda: GET
# Čo robí: Načíta prevody.json a vráti celý zoznam pre Frontend B.

@app.route("/api/historia-prevodov")
def historia_prevodov():
    """
    API endpoint — vráti históriu všetkých prevodov zo súboru prevody.json.
    Ak súbor neexistuje, vráti prázdny zoznam.

    Výstup (JSON):
        [
            {"hodnota": 25, "typ": "c_to_f", "vysledok": 77.0, "popis": "...", "cas": "..."},
            ...
        ]
    """
